...
class Content(models.Model):  # We want comment to have a foreign key to all contents so we use all of them as one
    ...
    type = EnumField(ContentType, default=ContentType.ARTICLE, max_length=100)
    main_type = EnumField(MainContentType, default=MainContentType.TEXT, max_length=100)

    # no api
    visibility = EnumField(ContentVisibility, default=ContentVisibility.PUBLIC, max_length=100)
...
class ContentRelation(models.Model):
    ...
    type = EnumField(ContentRealtionType, default=ContentRealtionType.COMMENTED_ON, max_length=100)