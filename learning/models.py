...
class Mentoring(models.Model):
    student_info = models.ForeignKey(LearningInfo, null=False, blank=False, related_name="student_info",
                                     on_delete=models.CASCADE)
    mentor_info = models.ForeignKey(LearningInfo, null=False, blank=False, related_name="mentor_info",
                                    on_delete=models.CASCADE)
...
class QualificationInfo(models.Model):
    qualified_student_info = models.ForeignKey(LearningInfo, null=False, blank=False,
                                               related_name="qualified_student_info", on_delete=models.CASCADE)
    qualifier = models.ForeignKey(settings.AUTH_USER_MODEL, default=1, on_delete=models.CASCADE)
    score = models.DecimalField(max_digits=4, decimal_places=2)

    def __str__(self):
        return self.student.username + " | " + self.qualifier.name + " | " + self.learning_field.title + " | " + str(
            self.score)
